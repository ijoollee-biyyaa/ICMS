...
public class DistrictDepartmentRepository(IcmsDbContext dbContext) : IDistrictDepartmentRepository
{
    ...
    public Task<DepartmentRow?> GetDepartmentAsync(long departmentId, CancellationToken ct) =>
        dbContext.Departments.AsNoTracking()
            .Where(d => d.Id == departmentId && d.ChurchId == null)
            .Select(d => new DepartmentRow(d.Id, d.Name, d.Type, d.HeadEmployeeId,
                d.HeadEmployee != null ? d.HeadEmployee.Position : null,
                d.Employees.Count))
            .SingleOrDefaultAsync(ct);

    public Task<Department?> GetTrackedDepartmentAsync(long departmentId, CancellationToken ct) =>
        dbContext.Departments
            .SingleOrDefaultAsync(d => d.Id == departmentId && d.ChurchId == null, ct);

    public Task<List<DepartmentRow>> GetDepartmentsAsync(
        string? search, int page, int pageSize, CancellationToken ct) =>
        dbContext.Departments.AsNoTracking()
            .Where(d => d.ChurchId == null
    ...
    public Task<int> CountDepartmentsAsync(string? search, CancellationToken ct) =>
        dbContext.Departments.AsNoTracking()
            .CountAsync(d => d.ChurchId == null
    ...
    public Task<bool> HeadIsOfficeEmployeeAsync(long employeeId, CancellationToken ct) =>
        dbContext.Employees.AsNoTracking()
            .AnyAsync(e => e.Id == employeeId && e.ChurchId == null, ct);

    public Task<bool> EmployeeIsOfficeEmployeeAsync(long employeeId, CancellationToken ct) =>
        dbContext.Employees.AsNoTracking()
            .AnyAsync(e => e.Id == employeeId && e.ChurchId == null, ct);
