...
public class CreateChurchValidator : AbstractValidator<CreateChurchRequest>
{
    public CreateChurchValidator()
    {
        ...
        RuleFor(x => x.Type)
            .IsInEnum().WithMessage("Church type is invalid.");

        RuleFor(x => x.ParentChurchId)
            .NotNull().When(x => x.Type == ChurchType.Daughter)
            .WithMessage("A daughter church must have a parent church.");

        RuleFor(x => x.ParentChurchId)
            .Null().When(x => x.Type == ChurchType.Local)
            .WithMessage("A local church cannot have a parent church.");
        ...
        RuleFor(x => x.AdminFirstName)
            .NotEmpty().WithMessage("Church admin first name is required.");

        RuleFor(x => x.AdminFatherName)
            .NotEmpty().WithMessage("Church admin father name is required.");

        RuleFor(x => x.AdminGrandfatherName)
            .NotEmpty().WithMessage("Church admin grandfather name is required.");
    }
}